...
var getDatesRangeArray = function() {
	...
	var string1 = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=";
...
function buyStock(){
	...
	totalPrice = parseFloat(totalPrice.slice(1,totalLength+1));
	...
	if(totalPrice == 0){
		...
		var history = currentDate + " " + ticker+" Market Buy: " + shareNum + " shares at $" + sharePrice + " -- Total: $" + totalPrice + "<br/>";
...
function next(){
	...
	if((index+1) == portfolioDays.length){
		alert("THE END");
	}else{
		...
		currentDate = portfolioDays[index+1];
		document.querySelector("#date").innerHTML = currentDate;
		document.querySelector("#stockDate").innerHTML = currentDate;
		...
		for (var i =0; i < portfolioTickers.length ; i++){
			...
			currentPrice = Prices[ticker][currentDate];
			currentPrice = parseFloat(currentPrice).toFixed(2);
			currentPrice = parseFloat(currentPrice);

			var currenTotal = currentPrice * shareNum;
		...
		portfolioValue = stockTotalValue + buyingPower;
		
		document.querySelector("#portfolioValue").innerHTML = portfolioValue.toFixed(2);

		getDate = moment(currentDate).valueOf()
...
tradeApp.controller('tradeController', function($scope){
	...
	$scope.buyTotal = function(){
		var buyTotal = 0;
		buyTotal = currentPrice * $scope.buyNum;
